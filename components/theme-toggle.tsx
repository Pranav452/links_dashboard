...
// Animated sun/moon theme switch (Solar-style path morph).
export function AnimatedThemeToggle({ className }: { className?: string }) {
  ...
  if (!mounted) {
    return (
      <button className={cn("inline-flex items-center justify-center p-2", className)} disabled aria-hidden>
  ...
  return (
    <button
      ...
      className={cn("inline-flex items-center justify-center p-2 transition-colors", className)}
